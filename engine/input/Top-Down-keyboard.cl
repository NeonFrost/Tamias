...
(defun level-keys-down (key)
  (case key
    ...
    (:scancode-left (setf (vector-3d-x (entity-vector player)) (* (round (/ *screen-width* 64)) -1)))
    (:scancode-right (setf (vector-3d-x (entity-vector player)) (round (/ *screen-width* 64))))
    (:scancode-up (setf (vector-3d-y (entity-vector player)) (* (round (/ *screen-height* 64)) -1)))
    (:scancode-down (setf (vector-3d-y (entity-vector player)) (round (/ *screen-height* 64))))
    ))
...
(defun level-keys-up (key)
  (case key
    ...
    (:scancode-up (stop-moving player))
    (:scancode-down (stop-moving player))
    ))
