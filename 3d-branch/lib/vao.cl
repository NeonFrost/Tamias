...
(defun create-spatial-vao (model)
  (loop for object in (model-objects model)
       do (let* ((buffers (gl:gen-buffers 2))
...
;;		 (vao (gl:gen-vertex-array))
		 ...
		 (cur-ind 0)
		 (3d-offset 0))
	     (setf index-length (* (car (array-dimensions (object-polygons object))) 3)
		   index-array (gl:alloc-gl-array :unsigned-short index-length)
		   position-array (gl:alloc-gl-array :float (* index-length 3)))
	     (loop for polygon below (car (array-dimensions (object-polygons object)))
		do (loop for point-idx below (car (array-dimensions (polygon-points (aref (object-polygons object) polygon))))
		      do (let ((point (aref (polygon-points (aref (object-polygons object) polygon)) point-idx)))
			   (setf (gl:glaref position-array 3d-offset)
				 (vertex-x (aref (model-vertices model) (point-position-index point)))
				 (gl:glaref position-array (1+ 3d-offset))
				 (vertex-y (aref (model-vertices model) (point-position-index point)))
				 (gl:glaref position-array (+ 3d-offset 2))
				 (vertex-z (aref (model-vertices model) (point-position-index point)))
				 (gl:glaref index-array cur-ind) cur-ind)
			   (princ (gl:glaref position-array 3d-offset)) (princ "   ") (princ (gl:glaref position-array (1+ 3d-offset)))
			   (princ "    ") (princ (gl:glaref position-array (+ 3d-offset 2))) (fresh-line)
			   (incf 3d-offset 3)
			   (incf cur-ind))))
...
#|	     (gl:bind-vertex-array vao)
	     ...
	     (setf ;;(object-vao object) vao
		   (object-ibo object) index-buffer
		   (object-vbo-ids object) (list position-buffer))
	     )))
...
;      (setf index-array (gl:alloc-gl-array :unsigned-short index-length))
    ;;      (gl:bind-buffer :element-array-buffer index-buffer)
  (loop for object in (model-objects model)
	do (let* ((buffers (gl:gen-buffers 4))
		  (position-buffer (elt buffers 0))
		  (normal-buffer (elt buffers 1))
		  (uv-buffer (elt buffers 2))
		...
		do (loop for point-idx below (car (array-dimensions (polygon-points (aref (object-polygons object) polygon))))
		      do (let ((point (aref (polygon-points (aref (object-polygons object) polygon)) point-idx)))
			   (setf (gl:glaref position-array 3d-offset) (aref (vertex-values (aref (model-vertices model) (point-position-index point))) 0)
				 (gl:glaref normal-array 3d-offset) (aref (vertex-values (aref (model-normal-vertices model) (point-normal-index point))) 0)
				 (gl:glaref uv-array 2d-offset) (aref (vertex-values (aref (model-texture-vertices model) (point-texture-index point))) 0)
				 (gl:glaref position-array (1+ 3d-offset)) (aref (vertex-values (aref (model-vertices model) (point-position-index point))) 1)
				 (gl:glaref normal-array (1+ 3d-offset)) (aref (vertex-values (aref (model-normal-vertices model) (point-normal-index point))) 1)
				 (gl:glaref uv-array (1+ 2d-offset)) (aref (vertex-values (aref (model-texture-vertices model) (point-texture-index point))) 1)
				 (gl:glaref position-array (+ 3d-offset 2)) (aref (vertex-values (aref (model-vertices model) (point-position-index point))) 2)
				 (gl:glaref normal-array (+ 3d-offset 2)) (aref (vertex-values (aref (model-normal-vertices model) (point-normal-index point))) 2)
				 (gl:glaref index-array cur-ind) cur-ind)
			   (incf 2d-offset 2)
			   (incf 3d-offset 3)
			   (incf cur-ind))))
	     ...
	     (setf (object-vao object) vao
		   (object-ibo object) index-buffer
		   (object-vbo-ids object) (list position-buffer normal-buffer uv-buffer))
	     )))

    (defun free-vao (model)
      )
